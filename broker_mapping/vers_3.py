...
for broker in target_global_name:
    ...
    print "=================================================================="
    print "### {0} ### - {1}".format(norm_broker, len(brokers_data))
    if(len(brokers_data) == 0):
        ...
        mapped.loc[mapped['Broker'].str.match(norm_broker), 'comments'] = "NO BROKER DATA AVAILABLE"
        mapped.loc[mapped['Broker'].str.match(norm_broker), 'MATCHED?'] = False
        continue
    target_broker = usi_target.loc[usi_target["Broker"].str.match(brokers_data["Global Ultimate Business Name"].unique()[0])]
    for city in target_cities:    
        targets = target_broker[(target_broker["Broker"].str.match(norm_broker)) & (target_broker.City.str.upper() == city)]
        brokers = brokers_data[(brokers_data["Global Ultimate Business Name"].str.match(norm_broker)) & (brokers_data['Physical City'].str.upper() == city)]
        ...
        if(len(brokers) == 0):
            ...
            mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'targets/brokers'] = str(len(targets)) + "/"+str(len(brokers))
            mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'comments'] = "NO BROKERS FOR THE CITY"
            mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'MATCHED?'] = False
            continue
        if(len(targets) == 0):
        ...
        try: 
            ...
            if((len(brokers) == 1) & (len(targets) == 1)):
                ...
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'targets/brokers'] = str(len(targets)) + "/"+str(len(brokers))
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'comments'] = "ONE BROKER MATCHED"
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'broker address'] = brokers['Physical Street Address Line 1'].values[0]
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'broker zip'] = brokers['Physical Zip (All)'].values[0]
                ...
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'broker name'] = brokers['Business Name'].values[0]
                ...
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'Duns number'] = brokers['Duns Number'].values[0]
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'Duns name'] = brokers['Business Name'].values[0]
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'Global Duns Number'] = brokers['Global Ultimate Duns Number'].values[0]
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'Global Business Name'] = brokers['Global Ultimate Business Name'].values[0]
                mapped.loc[(mapped['City'].str.upper() == city) & (mapped["Broker"].str.match(norm_broker)), 'MATCHED?'] = True
                continue
